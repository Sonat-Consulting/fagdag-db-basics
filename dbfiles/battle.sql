...
BEGIN
        ...
        RAISE NOTICE 'I do not know his last name.  I assume it doesnt matter, I will just set that to NULL';
        RAISE NOTICE ' ';
        RAISE NOTICE 'Narrator: It does.';

BEGIN
    INSERT INTO person
    values(3250003, 123456789101112, 4000, 'F', 'Martin', Null);
    EXCEPTION
            WHEN OTHERS THEN
...
BEGIN
    INSERT INTO city
    values
        (9999, 'Slopergrad', 'XXX', 'District9', 1);
    EXCEPTION
            WHEN OTHERS THEN
...
BEGIN
    INSERT INTO person
    values(3250001, 8983027730798, 4000, 'M', 'Martin', 'Sloper');
...
BEGIN
        state :=
(select coalesce(max(game_state.state),0)
from game_state);

CASE state
            when 0 THEN call usp_Battle1
(); -- missing not null
            when 1 THEN call usp_Battle2
(); -- missing foreign key
            when 2 THEN call usp_Battle3
(); -- inserting logical duplicate
            when 3 THEN call usp_Battle4
(); -- constraint value
            else
